...
function Table() {
  ...
  const clickPreviousNext = (type) => () =>{
    ...
    const params = type === 'previous' ? previousPage.split('?')[1] : nextPage.split('?')[1]
    dataFetch(params)
  ...
  return (
    ...
    <div class="buttons-container">
          <Button variant="primary" disabled={charactersDataResponse ? !charactersDataResponse.info.previousPage : true} onClick={clickPreviousNext('previous')}>Previous</Button>
          <Button variant="primary" disabled={charactersDataResponse ? !charactersDataResponse.info.nextPage : true} onClick={clickPreviousNext('next')}>Next</Button>
    </div>
    </>
  );
}
